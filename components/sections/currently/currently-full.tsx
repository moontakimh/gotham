...
export function CurrentlyFull() {
  const { label, title, blocks, callout } = currentlyContent;

  return (
    <EditorialSection aria-label="Currently" spacing="chapter">
      <EditorialContainer>
        <EditorialGrid>
          <EditorialCol
            as="p"
        ...
        <EditorialGrid className="mt-[clamp(4rem,10vh,7rem)]">
          <EditorialCol
            as="p"
            span="body-offset"
            className="font-body text-base leading-snug text-accent-signal max-md:ml-[clamp(1.5rem,6vw,3rem)] md:text-lg"
          >
            {callout}
          </EditorialCol>
        </EditorialGrid>
